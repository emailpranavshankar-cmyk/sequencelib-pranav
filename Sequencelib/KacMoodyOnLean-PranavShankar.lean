...
theorem PrimeDivisibilityYIf
    ...
    ∃ n : ℕ, n > 0 ∧ (p : ℤ) ∣ KacMoodyY (n, k) := by
         ...
         · simp_rw [X_to_Y]
           ...
           have numUsedNat : ∃ n : ℕ, n = (orderOf α - 1) / 2 := by
                 obtain ⟨m, hm⟩ := hodd
                 use m
                 omega
           use (orderOf α - 1) / 2
           constructor
           · have orderNot1 : orderOf α ≠ 1 := by
              intro all_one
              rw [orderOf_eq_one_iff] at all_one
              rw [all_one] at hroot1
              ring_nf at hroot1
              have k2 : (k : ZMod p) = 2 := by
                simp at hroot1
                linear_combination -hroot1
              rw [k2] at hroot2
              have b1 : (β : ZMod p) = 1 := by
                have sq : ((β : ZMod p) - 1)^2 = 0 := by linear_combination hroot2
                have minus := sq_eq_zero_iff.mp sq
                linear_combination minus
              have bunit : β = 1 := Units.val_eq_one.mp b1
              have all_equal : α = β := by rw [all_one, bunit]
              exact absurd all_equal hnzero
             obtain ⟨m, hm⟩ := hodd
             omega
           · have e1 := explicit ((orderOf α - 1) / 2)
             have e2 := explicit ((orderOf α - 1) / 2 + 1)
             rw [← ZMod.intCast_zmod_eq_zero_iff_dvd]
             push_cast
             rw [e1, e2]
             have alphabet : (α : ZMod p) * β = 1 := by
               have factor : ((α : ZMod p) - β) * (α * β - 1) = 0 := by
                 linear_combination β * hroot1 - α * hroot2
               rcases mul_eq_zero.mp factor with leftside | rightside
               · exact absurd (Units.ext (sub_eq_zero.mp leftside)) hnzero
               · exact sub_eq_zero.mp rightside
             have inv : (β : ZMod p) = ((α : ZMod p))⁻¹ := by
                field_simp
                rw[mul_comm]
                exact alphabet
             rw[inv]
             obtain ⟨m, hm⟩ := hodd
             have m_form : m = (orderOf α - 1) / 2 := by
                omega
             have horder : (α : ZMod p) ^ (2 * m + 1) = 1 := by
                have CASTER := pow_orderOf_eq_one α
                rw [← hm]
                have cast_npow: (↑(α ^ orderOf α) : ZMod p) = ↑(1 : (ZMod p)ˣ) := by exact_mod_cast CASTER
                rw [Units.val_pow_eq_pow_val] at cast_npow
                exact cast_npow
             rw [← m_form]
             ring_nf
             field_simp
             rw [div_eq_zero_iff]
             left
             have little_factors : ((α : ZMod p) + 1) * ((α : ZMod p)^(m+1) - ((α : ZMod p)⁻¹)^m) = 0 := by
               have little_horder : (α : ZMod p)^(m+1) = ((α : ZMod p)⁻¹)^m := by
                  have hne : (α : ZMod p) ≠ 0 := Units.ne_zero α
                  rw [inv_pow]
                  rw [eq_comm, ← mul_right_inj' (pow_ne_zero m hne)]
                  have rmult :  (↑α ^ m)⁻¹ = ↑α ^ (m + 1) := by
