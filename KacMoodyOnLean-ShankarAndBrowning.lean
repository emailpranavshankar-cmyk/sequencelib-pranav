...
def KacMoodyY : ℕ × ℤ → ℤ -- definition of Y
| (0, _) => 1
| (1, k) => k + 1
...
lemma YExplicitFormulaRingSafe {R : Type*} [CommRing R] (k : ℤ) (w1 w2 : R) -- Explicit formula for Y with multiplier to avoid division - this ring cannot always be embedded in a field
  (hw1 : w1^2 - (k : R) * w1 + 1 = 0)
...
-- Y defined in lines 18 - 21, explicit formula with multiplier stated and proved in lines 217-236
variable (k : ℤ)
...
-- beta is a root of the characteristic polynomial
...
lemma alphabet_sum_lemma  (pod : p ≠ 2) (α β : (O_K_mod_p)ˣ) (hdiff : α ≠ β)
  ...
  -- beta is just the inverse of alpha
...
theorem PrimeDivisibilityY
  ...
  · have h_sq : (k : ZMod p)^2 = (2 : ZMod p)^2 := by
    ...
    · exfalso
      contradiction
  · constructor -- Unfold the iff, now prove that odd order implies p divides a term
    · intro hodd
      obtain ⟨ord, hord⟩ := hodd -- expressing the order as 2 * ord + 1
      use ord -- Y (ord) is divisible by p
      constructor
      · by_contra  -- First prove (using contradiction) that ord is positive
        have ord_zero : ord = 0 := by rw[Nat.eq_zero_of_not_pos this]
        rw [ord_zero] at hord
        ring_nf at hord
        have alpha_root: α = 1 := by rw[orderOf_eq_one_iff.mp hord] -- if the order is 0, alpha must be 1
        ...
        (k : ZMod p) = n := by -- going from O_K_mod_p to ZMod p
      ...
      · suffices h : ((α : O_K_mod_p) - (β : O_K_mod_p)) * (KacMoodyY (ord, k) : O_K_mod_p) = 0 by -- Main branch of first case: proving p divides Y(ord)
        ...
        have ord_eqn: (α : O_K_mod_p)^ (2 * ord + 1) * ((α : O_K_mod_p) + 1) - (α : O_K_mod_p) * (α⁻¹ + 1) = 0 := by
          ...
          simp
        have ord_eqn_lite : α⁻¹ ^ (ord + 1) * ((α : O_K_mod_p)^ (2 * ord + 1) * ((α : O_K_mod_p) + 1) - (α : O_K_mod_p) * (α⁻¹ + 1)) = 0 := by
          rw [ord_eqn]
          ring
        ring_nf
        ring_nf at ord_eqn_lite
        norm_cast
        norm_cast at ord_eqn_lite
        group
        group at ord_eqn_lite
        push_cast
